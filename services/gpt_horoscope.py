...
def generate_horoscope(sign: str, day: str = "today", detailed: bool = False):
    ...
    try:
        ...
        if detailed:
            ...
            Используй эмодзи для каждого раздела. Текст должен быть структурированным и информативным."""
        else:
            ...
            Текст должен быть кратким, но информативным. Используй эмодзи для каждого пункта."""

        # Генерируем гороскоп
        logger.info(f"Генерация гороскопа для {sign} на {date_str}")
        gpt_text = generate_text_with_system(system_prompt, user_prompt)
        
        # Добавляем лунный календарь только для подробного гороскопа
        if detailed:
            try:
